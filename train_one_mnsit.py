...
def main():
    ...
    for step in range(MAX_STEPS):
        ...
        # 1. THE HONEST PREDICTION PEEK (Unclamped)
        predicted_label = -1
        is_correct = False
        if graph.current_states is not None:
            with torch.no_grad():
                peek_states = graph.current_states.clone()
                for _ in range(3): # Let the electricity flow to the motor nodes
        ...
        s_surp = torch.nan_to_num(surprises, nan=0.0).tolist() if surprises is not None else [0.0]*graph.num_nodes
        s_raw  = torch.nan_to_num(raw_surprises, nan=0.0).tolist() if raw_surprises is not None else [0.0]*graph.num_nodes
        s_coor = torch.nan_to_num(graph.coords.detach().cpu(), nan=0.0).tolist()
        ...
        chunk_frames.append({
            ...
            'raw_surprises':  compress_list(s_raw, 3),

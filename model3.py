...
class Pos():
    def __init__(self, x, y):
        self.x = x
        self.y = y
    to_tuple=lambda self: (self.x, self.y)
...
class TiledGraph(nn.Module):
    ...
    def bucketize(self):
        for node in self.nodes.values():
            c = node.get_bucket(self.bucket_size)
            key = c.to_tuple()

            if key not in self.buckets:
                self.buckets[key] = set()

            self.buckets[key].add(node.id)
            self.node_bucket[node.id] = key